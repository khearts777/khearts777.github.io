...
<head>
<meta charset="UTF-8">
<title>K-Hearts Official Fan Site</title>
...
nav{background:#ff99cc;padding:15px;position:sticky;top:0;z-index:100;}
nav button{margin:5px;padding:8px 15px;background:#fff;color:#ff66b2;border:none;border-radius:20px;cursor:pointer;}
...
.avatar-container{position:relative;width:150px;height:300px;margin:0 auto;}
#head{width:80px;height:80px;background:#fddac4;border-radius:50%;position:absolute;top:0;left:35px;}
#top{width:80px;height:80px;background:pink;position:absolute;top:80px;left:35px;border-radius:10px;}
#bottom{width:80px;height:80px;background:white;position:absolute;top:160px;left:35px;border-radius:10px;}
#shoes{width:80px;height:20px;background:black;position:absolute;top:240px;left:35px;border-radius:5px;}
#handAccessory{position:absolute;top:160px;left:90px;font-size:30px;}
...
#chatInput{width:80%;padding:8px;border-radius:10px;border:1px solid #ff66b2;}
</style>
</head>
...
<!-- PROMPT USER NAME -->
<script>
let data = JSON.parse(localStorage.getItem("kheartsData")) || {coins:0, avatar:"🙂", outfit:"Basic", hasLightstick:false, username:"", leaderboard:[]};
...
let avatarPieces={head:"#fddac4",top:"pink",bottom:"white",shoes:"black",accessory:""};
...
document.getElementById("top").style.background=avatarPieces.top;
...
heart.innerText="💖";heart.className="heart";heart.style.left=Math.random()*90+"%";
...
h.innerText="💖";h.style.position="absolute";h.style.left=Math.random()*90+"%";h.style.top="0px";h.style.cursor="pointer";
h.onclick=function(){data.coins+=10;updateDisplay();saveData();h.remove();}
area.appendChild(h);
let fall=setInterval(()=>{let top=parseInt(h.style.top);if(top>=180){h.remove();clearInterval(fall);}else{h.style.top=top+2+"px";}},20);}
}
...
if(i===correct){data.coins+=20;updateDisplay();saveData();div.innerHTML="Correct! Badge "+badge;}else{div.innerHTML="Wrong! Try again!";}};div.appendChild(btn);});
}
...
if(name=="🎀 Coquette"){avatarPieces.top="pink";avatarPieces.bottom="white";avatarPieces.accessory="🎀";}
if(name=="🐧 Penguin"){avatarPieces.top="#aee1f9";avatarPieces.bottom="#f0f0f0";avatarPieces.accessory="🐧";}
if(name=="👑 Queen"){avatarPieces.top="gold";avatarPieces.bottom="black";avatarPieces.accessory="👑";}
...
/* LEADERBOARD */
function updateLeaderboard(){let list=document.getElementById("leaderboardList");list.innerHTML="";data.leaderboard.forEach(p=>{let div=document.createElement("div");div.innerText=p.name+" — "+p.score+" coins — "+titles[Math.floor(p.score/50)>9?9:Math.floor(p.score/50)];list.appendChild(div);});}
updateLeaderboard();

/* FAN CHAT */
function sendMessage(){let input=document.getElementById("chatInput");let msg=input.value; if(!msg)return;
let chat=document.getElementById("chatBox");
let userMsg=document.createElement("div");userMsg.innerText=data.username+": "+msg;chat.appendChild(userMsg);
let idolMsg=document.createElement("div");idolMsg.innerText="K-Hearts: "+["Thanks KAES!","You're amazing!","Keep shining!"][Math.floor(Math.random()*3)];chat.appendChild(idolMsg);
